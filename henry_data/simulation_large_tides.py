...
def build_and_run_henry(
    ...
    if not dynamic_tides:
        ...
        # 0.5m amplitude (1.0m total swing)
        tidal_heads = ghb_head + 0.5 * np.cos(2 * np.pi * times / 0.5)

        # Name records for each layer's specific head, conductance, and concentration
        h_names = [f"h_lay{k}" for k in range(nlay)]
        cond_names = [f"cond_lay{k}" for k in range(nlay)]
        ...
        for i, t in enumerate(times):
            ...
            for k in range(nlay):
                ...
                if h_t >= lay_bot:
                    # Tide is at least partially wetting this cell
                    row_h.append(h_t)
                    row_cond.append(float(ghbcond[k]))
                    row_c.append(35.0 if h_t >= lay_top else 35.0)
                else:
                    # Tide has fallen below this cell; disconnect it
                    row_h.append(lay_bot + 1e-4) # Clamp head safely above cell bottom
                    row_cond.append(0.0)         # Zero conductance isolates the boundary
                    row_c.append(0.0)
                    
            # Combine columns: [Time, h0..hN, cond0..condN, c0..cN]
            row.extend(row_h)
            row.extend(row_cond)
            row.extend(row_c)
            ts_data.append(tuple(row))

        # Each layer references its strictly individual h, cond, and c time series
        ghb_spd = {0: [[(k, 0, ncol - 1), f"h_lay{k}", f"cond_lay{k}", f"c_lay{k}"] for k in range(nlay)]}
        ...
        all_ts_names = h_names + cond_names + c_names
        ghb.ts.initialize(
            filename="ghb_ts.ts",
            timeseries=ts_data,
            time_series_namerecord=all_ts_names,
            interpolation_methodrecord=["linearend"] * len(all_ts_names),
        )

        # Output specific concentration variable for saving later
        cinlet_ts = np.array([[35.0 if h_t >= (top if k == 0 else botm[k-1]) else 0.0 for k in range(nlay)] for h_t in tidal_heads])
